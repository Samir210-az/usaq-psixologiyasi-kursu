...
// ---- Session Modal Engine ----
// Each page must define its own SESSIONS array and TOTAL_SESSIONS value
function renderSession(s){
  const total = (typeof TOTAL_SESSIONS !== 'undefined') ? TOTAL_SESSIONS : s.num;
  let html = `<span class="m-tag">Session ${s.num} / ${total}</span><h3 class="m-title">${s.title}</h3>`;
  if(s.about){
  ...
  html += `<div class="m-nextprev">
    <div class="m-navbtn ${s.num<=1?'disabled':''}" onclick="openSession(${s.num-1})">◂ Previous session</div>
    <div class="m-navbtn ${s.num>=total?'disabled':''}" onclick="openSession(${s.num+1})">Next session ▸</div>
  </div>`;
  return html;
}
